...
def convert_to_structured_dict(result):
    ...
    if isinstance(result, ParseResults):
        ...
        if len(named_keys) > 1:
            ...
            # For simple BGP at the beginning
            if 'bgp' in named_keys:
...
parse_and_show_results(where_clause, "WHERE { ?s ?p ?o . }", "simple WHERE clause")
parse_and_show_results(
    where_clause, 
    "WHERE { ?s ?p ?o . FILTER(?o > 5) }", 
    "WHERE clause with filter"
)
parse_and_show_results(
    where_clause, 
